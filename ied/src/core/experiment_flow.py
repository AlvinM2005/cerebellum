...
def run() -> None:
    ...
    record_hands(screen)
    cfg.VERSION = 1
    logger.info("Participant ID = %s | VERSION = %s", cfg.PID, cfg.VERSION)
    ...
    while True:
        ...
        if current_page == cfg.PRACTICE1:
            ...
            cfg.PHASE_START_TIME = _ts()
            cfg.PHASE_END_TIME = None
            run_single_stimulus_phase(screen, "P1", stimuli, event_handler, True)
            cfg.PHASE_END_TIME = _ts()
            ...
            cfg.PHASE_START_TIME = _ts()
            cfg.PHASE_END_TIME = None
            run_single_stimulus_phase(screen, "P2", stimuli, event_handler, False)
            cfg.PHASE_END_TIME = _ts()
