...
def run() -> None:
    ...
    try:
        ...
        # Load assets
        event_handler = EventHandler()
        INSTRUCTIONS, PRACTICE_INSTRUCTIONS, TEST_INSTRUCTIONS = load_instructions()
        ...
        # 4) instruction practice
        screen = _show_instruction_page(screen, PRACTICE_INSTRUCTIONS, event_handler)
        ...
        # 7) instruction test
        screen = _show_instruction_page(screen, TEST_INSTRUCTIONS, event_handler)

        # 8) test block (no feedback)
        screen = run_test(screen, "test", STIMULI, event_handler)

        # 9) instruction 5
        screen = _show_end_page(screen, INSTRUCTIONS[4], event_handler)
