...
def run_synchronized(start_tick, target_key, max_key_press, stimulus_path):
    pygame.event.clear()
    ...
    while valid_key_pressed_num < max_key_press:
        # Play sound
        if len(sound_ticks) != len(key_responses):
            key_responses.append([None, None])
            # key_responses.append([None, None])
            print(f"No key press at sound {sound_index}")
        sound_index += 1
        ...
        print()
        print(f"Sound {sound_index} played!")
...
def run_self_paced(start_tick, target_key, max_key_press):
...
def run_trial(start_tick, target_key, max_synchronized_key_press, max_self_paced_key_press, stimulus_path):
    synchronized_sound_ticks, synchronized_key_responses = run_synchronized(start_tick, target_key, max_synchronized_key_press, stimulus_path)
    self_paced_key_responses = run_self_paced(start_tick, target_key, max_self_paced_key_press)
...
- trial_type: True (if all interval_ms of self-paced tappings in given trials is in [MIN_SELF_PACED_INTERVAL, MAX_SELF_PACED_INTERVAL]) / False (otherwise)
- key_correct: "v" / "m"
- group: i.e. YC / CD [first two text of participant_id]
'''

def single_trail(block, start_tick, target_key, results, participant_id, trial = None, csv_file = None):
    ...
    (synchronized_sound_ticks, 
     ...
     self_paced_key_responses) = run_trial(start_tick, 
                                           target_key,
                                           NUM_SYNCHRONIZED, 
                                           NUM_SELF_PACE, 
                                           STIMULUS_PATH_900)
    ...
    for i in range(len(response_ticks)):
        single_trail_result = [participant_id, participant_id[0:2], block, trial, len(results) + 1, type[i], ISI_ms, type[i], ISI_ms, os.path.abspath(STIMULUS_PATH_900),
                               synchronized_sound_ticks[i], response_ticks[i], intervals[i], trial_type, key_correct, start_time, end_time]
