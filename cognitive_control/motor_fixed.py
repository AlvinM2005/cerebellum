...
def key_logging(duration_ms, screen):
    pygame.display.flip()
    pygame.event.clear()
...
def run_trials(trials, response_time, isi_time, condition, read_trial, screen):
    ...
    for trial in trials:
        ...
        fixation_key_response, fixation_reaction_time = key_logging(fixation_time, screen)
        ...
        stimulus_key_response, stimulus_reaction_time = key_logging(response_time, screen)
        ...
        isi_key_response, isi_reaction_time = key_logging(isi_time, screen)
...
def run_m_segment1(screen, all_results, all_acc, next_segment_func):
    ...
    for i in range(0, PRACTICE1_2_PAGE):            
        if i == PRACTICE1_1_PAGE - 1:
            instruction_flow.append(("instruction", M_INSTRUCTION_PATH[i]))
