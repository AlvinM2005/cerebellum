...
def run_trials(trials, response_time, isi_time, condition, screen):
    ...
    for trial in trials:
        ...
        if phase.startswith("practice"):
            # Feedback
            if type == "no_go":
                correct = (fixation_key_response is None
                           and stimulus_key_response is None)
                timeout = False
            else:
                correct = (stimulus_key_response == key_correct)
                timeout = (fixation_key_response is None
                           and stimulus_key_response is None)
        ...
        # Store results
        avg_fixation_time = (M_AVG_FIXATION_TIME if condition == "motor" else SM_AVG_FIXATION_TIME)
