...
def _construct_stimuli_seuqnece() -> list[Path]:
    """
    Construct a stimulus sequence for 3back
...
def run_3back(
    ...
    for i in range(len(_stim_seq)):
        ...
        # Dummy stimuli (first 3)
        if i == 0 or i == 1 or i == 2:
            if result == "timeout":
                update_save(phase, condition, "3back", None, None, "correct", None, stim_id)

            else:
                update_save(phase, condition, "3back", response, None, "incorrect", reaction_time, stim_id)
