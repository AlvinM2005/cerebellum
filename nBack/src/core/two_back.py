# ./src/core/two_back.py
"""
2-back task execution with deterministic sequence generation.
...
from core.pull_stimuli import pull_stimuli_2back
...
def run_2back(
    ...
    """
    Execute 2-back block with fixed 3000ms timing per trial.
    ...
    """
    _stim_seq, _match_map = pull_stimuli_2back(trial_num)

    for i in range(len(_stim_seq)):
        stim_path = _stim_seq[i]
        stim_id = str(stim_path.stem)
        match = _match_map[i] if i > 1 else None
        ...
        if i < 2:
            # First two trials: any response is incorrect
        ...
        while (pygame.time.get_ticks() - trial_start) < total_window:
            ...
            if _is_practice and not feedback_shown and option_selected is None:
                ...
                elif not match and i > 1 and elapsed >= (total_window - cfg.FB_DURATION):
                    if feedback_start_time is None:
                        feedback_start_time = pygame.time.get_ticks()
                        feedback_type = "correct"
                        feedback_shown = True
        ...
        # Save trial data
        update_save(condition, "2back", response, correct_response, result, signal_detection, reaction_time, stim_id)
